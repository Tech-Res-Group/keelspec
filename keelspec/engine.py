...
Diagnostics carry stable PAC-NNN codes (see :mod:`keelspec.diagnostics`).
"""
...
_GENERATED_NOTE = (
    "AUTO-GENERATED by KeelSpec (keelspec build). Do not edit by hand — edit the "
...
def validate(config: Config, root: str | pathlib.Path = ".", registry=None) -> Report:
    ...
    out = pathlib.Path(root) / config.output
    if not out.exists():
        report.add("PAC-060", f"{config.output} is missing — run: keelspec build")
    else:
        ...
        if committed != expected:
            report.add(
                ...
                f" — run: keelspec build (and commit it)",
            )
    ...
    if registry:
        ...
        for rel, renderer in registry.extra_outputs:
            ...
            if not ep.exists():
                report.add("PAC-060", f"{rel} is missing — run: keelspec build")
            elif ep.read_text(encoding="utf-8") != expected:
                report.add("PAC-060", f"{rel} is stale — run: keelspec build (and commit it)")
